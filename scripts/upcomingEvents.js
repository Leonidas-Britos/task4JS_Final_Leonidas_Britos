    ...
    inputCheck.addEventListener("change", function tarjetaSeleccionada() {
      ...
      if (ArrInputsChecked.length === 0) {
        ...
        if (inputBuscados.length == 0) {
          ...
          let categoriasSeleccionadas = inputBuscados.filter(upcomingFiltrado => ArrInputsChecked.includes(upcomingFiltrado.category));
          imprimirCards(categoriasSeleccionadas, '.cards_upcomingEvents');
...
const busquedaPorNombreyCoincidencia = () => {
  ...
  form.addEventListener('submit', (e) => {
    ...
    if (ArrInputsChecked.length === 0) {
      const coincidencias = upcomingFiltrado.filter(upcomingFiltrado => upcomingFiltrado.name.toLowerCase().includes(busqueda) || upcomingFiltrado.description.toLowerCase().includes(busqueda)
      );
      ...
      const categoriasSeleccionadas = upcomingFiltrado.filter(upcomingFiltrado => ArrInputsChecked.includes(upcomingFiltrado.category));
      //Almaceno los resultados en la variable coincidencias
      const coincidencias = categoriasSeleccionadas.filter(upcomingFiltrado => upcomingFiltrado.name.toLowerCase().includes(busqueda) || upcomingFiltrado.description.toLowerCase().includes(busqueda));
